...
import { FaWhatsapp, FaSearch, FaTag } from 'react-icons/fa';
...
function SkeletonCard() {
  return (
    <div className="bg-white rounded-2xl overflow-hidden border border-sand-300 animate-pulse">
      <div className="h-52 bg-sand-200" />
      <div className="p-5 space-y-3">
        <div className="h-5 bg-sand-200 rounded-full w-3/4" />
        <div className="h-3 bg-sand-200 rounded-full w-1/3" />
        <div className="h-3 bg-sand-200 rounded-full w-full" />
        <div className="h-3 bg-sand-200 rounded-full w-5/6" />
        <div className="flex gap-2 pt-2">
          <div className="h-6 bg-sand-200 rounded-full w-16" />
          <div className="h-6 bg-sand-200 rounded-full w-16" />
        </div>
        <div className="h-11 bg-sand-200 rounded-xl mt-2" />
      </div>
    </div>
  );
}
...
export default function Products() {
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter(
        (p) => p.name.toLowerCase().includes(q) || p.nameNepali?.toLowerCase().includes(q) || p.description.toLowerCase().includes(q)
      );
    }
    return list;
  ...
  const globalWhatsAppUrl = `https://wa.me/${whatsappNumber.replace(/\D/g, '')}?text=${encodeURIComponent(
    settings.whatsappDefaultMessage || 'Hello! I am interested in your rice products. Please share pricing and availability.'
  )}`;
  ...
  return (
    <>
      ...
      <div className="bg-[#25D366] text-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-3 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-3 text-sm">
            ...
            <p><span className="font-semibold">Quick inquiry?</span> Chat with us on WhatsApp — we respond within minutes.</p>
          </div>
          <a href={globalWhatsAppUrl} target="_blank" rel="noopener noreferrer"
      ...
      {/* ── Brands Highlight ── */}
      {brands.length > 0 && (
        <section className="section-pad-sm bg-sand-100">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <SectionReveal className="text-center mb-10">
              ...
              <h2 className="font-heading text-3xl font-bold text-ink-900">Trusted Brand Names</h2>
              <p className="text-ink-500 mt-2 max-w-xl mx-auto text-sm">Every grain carries the promise of a brand built on quality and care.</p>
            </SectionReveal>
      ...
      <div className="bg-white border-b border-sand-300 shadow-sm sticky top-[64px] md:top-[80px] z-30">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-3 space-y-2">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1 max-w-xs">
              <FaSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-400 text-sm" />
              <input type="text" placeholder="Search rice varieties..." value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-9 pr-4 py-2 text-sm border border-sand-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-400 bg-sand-50 transition" />
            </div>
              ...
              {CATEGORIES.map(({ key, label, emoji }) => (
                <button key={key} onClick={() => setActiveCategory(key)}
                  ...
                  {categoryCounts[key] !== undefined && (
                    <span className={`text-xs rounded-full px-1.5 py-0.5 leading-none font-bold ${activeCategory === key ? 'bg-white/20 text-white' : 'bg-sand-300 text-ink-500'}`}>
                      {categoryCounts[key]}
                    </span>
                  )}
                </button>
          ...
          {brands.length > 0 && (
            <div className="flex items-center gap-2 overflow-x-auto pb-0.5 scrollbar-none">
              <span className="text-xs font-semibold text-ink-400 uppercase tracking-wide flex-shrink-0 flex items-center gap-1">
                <FaTag className="text-xs" /> Brand:
              </span>
              <button onClick={() => setActiveBrand('all')}
                className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold transition ${activeBrand === 'all' ? 'bg-ink-900 text-white' : 'bg-sand-100 text-ink-600 hover:bg-sand-200'}`}>
                All Brands
              </button>
              {brands.map((brand) => {
                ...
                return (
                  <button key={brand._id} onClick={() => setActiveBrand(brand.name)}
                    ...
                    className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition ${active ? '' : 'bg-sand-100 text-ink-600 hover:bg-sand-200'}`}>
                    {brand.logo
                    ...
                    <span className={`text-xs rounded-full px-1.5 py-0.5 leading-none font-bold ${active ? 'bg-white/25 text-white' : 'bg-sand-300 text-ink-500'}`}>{count}</span>
                  </button>
                );
              })}
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          {loading ? (
            ...
            <div className="text-center py-24">
              ...
              <h3 className="font-heading font-bold text-ink-800 text-xl mb-2">No products found</h3>
            ...
            <>
              <p className="text-sm text-ink-400 mb-6">
                Showing <span className="font-semibold text-ink-700">{filtered.length}</span> product{filtered.length !== 1 ? 's' : ''}
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-7">
                {filtered.map((product, i) => (
                  <SectionReveal key={product._id} delay={Math.min(i * 50, 300)}>
                    <ProductCard product={product} whatsappNumber={whatsappNumber} brands={brands} />
                  </SectionReveal>
                ))}
              </div>
            </>
      ...
      <section className="section-pad bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <SectionReveal className="text-center mb-12">
            ...
            <h2 className="section-title mx-auto">Flexible Packaging Options</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {[
              ...
              <SectionReveal key={size} delay={i * 60}>
                <div className="bg-sand-100 hover:bg-sand-200 border border-sand-300 hover:border-gold-400 rounded-2xl p-5 text-center transition-all duration-300 cursor-default">
                  <div className="text-3xl mb-2">{icon}</div>
                  <div className="font-heading font-bold text-ink-800 text-lg">{size}</div>
                  <div className="text-ink-400 text-xs mt-1 leading-tight">{forText}</div>
                </div>
              </SectionReveal>
            ))}
          </div>
      ...
      <section className="section-pad-sm bg-ink-900 grain-overlay text-white">
        <div className="max-w-4xl mx-auto px-6 lg:px-8">
          <SectionReveal className="text-center">
            ...
            <h2 className="font-heading text-3xl font-bold text-white mb-4">Looking for Bulk or Wholesale?</h2>
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              ...
              <a href="/contact"
                className="inline-flex items-center gap-2 border-2 border-white text-white hover:bg-white hover:text-ink-900 font-semibold px-8 py-3.5 rounded-full transition-all duration-200">
                Send a Message
              </a>
            </div>
          </SectionReveal>
